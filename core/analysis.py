    ...
    while True:
        try:
            ...
            for symbol in symbols:
                ...
                if signal['trade_type'] == "Scalping" and signal['confidence'] < 80:
                    log(f"⏩ Skipped {symbol} (Scalping < 80%)")
                    continue
                elif signal['trade_type'] == "Normal" and signal['confidence'] < 90:
                    log(f"⏩ Skipped {symbol} (Normal < 90%)")
                    continue
                ...
                try:
                    ...
                    # Skip sideways
                    if signal['prediction'] == "SIDEWAYS":
                        log(f"⏩ Skipped {symbol} (Sideways prediction)")
                        continue
                    ...
                    if atr == 0:
                        log(f"⚠️ ATR missing for {symbol}, skipping TP/SL calc")
                        continue

                    multiplier = 1.3 if signal['trade_type'] == "Scalping" else 2.0

                    if signal['prediction'] == "LONG":
                        signal['tp1'] = round(price + atr * multiplier, 3)
                        signal['tp2'] = round(price + atr * multiplier * 2, 3)
                        signal['tp3'] = round(price + atr * multiplier * 3, 3)
                        signal['sl'] = round(price - atr * multiplier, 3)
                    else:  # SHORT
                        signal['tp1'] = round(price - atr * multiplier, 3)
                        signal['tp2'] = round(price - atr * multiplier * 2, 3)
                        signal['tp3'] = round(price - atr * multiplier * 3, 3)
