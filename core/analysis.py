...
def log_debug_info(signal):
    ...
    log(f"TP1: {signal['tp1']} ({signal['tp1_prob']}%) | TP2: {signal['tp2']} ({signal['tp2_prob']}%) | TP3: {signal['tp3']} ({signal['tp3_prob']}%)")
    log(f"SL: {signal['sl']} | Support: {signal.get('support')} | Resistance: {signal.get('resistance')}")
    ...
    while True:
        ...
        for symbol in symbols:
            ...
            try:
                ...
                buffer = atr * 1.5 if atr else signal["price"] * 0.01

                if direction == "LONG":
                    if signal["resistance"] and signal["resistance"] - signal["price"] > buffer:
                        signal["confidence"] += 5
                    else:
                        continue
                elif direction == "SHORT":
                    if signal["support"] and signal["price"] - signal["support"] > buffer:
                        signal["confidence"] += 5
                    else:
                        continue
                ...
                signal["tp1_prob"] = min(99, signal["confidence"] + 3)
                signal["tp2_prob"] = max(65, signal["confidence"] - 5)
                signal["tp3_prob"] = max(50, signal["confidence"] - 15)
